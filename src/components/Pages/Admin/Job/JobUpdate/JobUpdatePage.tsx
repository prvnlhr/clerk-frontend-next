...
import JobDescriptionEditor from "../JobCreate/JobDescriptionEditor";
import { EditJob } from "@/types/Application/applicationTypes";
...
interface JobUpdatePageProps {
  editJobData: EditJob;
}

const JobUpdatePage: React.FC<JobUpdatePageProps> = ({ editJobData }) => {
  ...
  useEffect(() => {
    if (editJobData) {
      ...
      reset({
        ...
        workType: editJobData.workType,
        salaryType: editJobData.salaryType,
        fixedSalary:
          editJobData.salaryType === "fixedAmount"
            ? Number(editJobData.salary?.replace("LPA", "00000"))
            : undefined,
        salaryFrom:
          editJobData.salaryType === "range"
            ? Number(editJobData.salaryRange?.split("-")[0] + "00000")
            : undefined,
        salaryTo:
          editJobData.salaryType === "range"
            ? Number(
                editJobData.salaryRange?.split("-")[1].replace("LPA", "00000")
              )
            : undefined,
        skills: editJobData.skills,
        experience: editJobData.yoe,
        jobDescription: editJobData.description || "",
      });

      // Set image previews if URLs exist
      if (editJobData.coverImageUrl) {
        setCoverImagePreview(editJobData.coverImageUrl);
      }
      if (editJobData.logo) {
        setCompanyLogoPreview(editJobData.logo);
      }
    }
  }, [editJobData, reset]);
  ...
  useEffect(() => {
    if (selectedSalaryType && editJobData) {
      if (selectedSalaryType !== editJobData.salaryType) {
        setValue("salaryFrom", undefined);
        setValue("salaryTo", undefined);
        setValue("fixedSalary", undefined);
      }
    }
  }, [selectedSalaryType, editJobData, setValue]);
  ...
  const onSubmit = (data: JobFormData) => {
    ...
    console.log("Form submitted:", latestData);
  };
      ...
      <div className="w-[100%] md:w-[60%] h-[95%] overflow-y-scroll hide-scrollbar bg-white p-[20px] rounded">
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="w-[100%] h-[50px] flex items-center border-b border-black/10 mb-[20px] sticky -top-[20px] bg-white z-3">
            <h1 className="text-2xl font-medium">Edit Job Post</h1>
          </div>
          ...
          <section className="w-full h-auto flex flex-col mt-[0px]">
            ...
            <div className="w-[100%] h-[140px]">
              ...
              <label className="relative w-[100%] h-[100px] flex items-center justify-center border-2 border-dashed border-[#D6DDEB] rounded cursor-pointer">
                {editJobData && !coverImagePreview && (
                  <Image
                    src={editJobData.coverImageUrl}
                    alt="Cover preview"
                    fill={true}
              ...
              <div className="relative w-[100%] h-[40px]">
                <label className="absolute rounded border-2 border-dashed border-[#D6DDEB] bg-gray-100 h-[80px] aspect-[1/1] flex items-center justify-center left-[20px] bottom-[0%] z-[2] cursor-pointer">
                  {editJobData && !companyLogoPreview && (
                    <Icon icon={editJobData.logo} className="w-full h-full" />
                  )}
                  <input
                    type="file"
                    accept="image/*"
                    ...
                    <div className="w-auto h-[30px] flex justify-start border border-[#D6DDEB] rounded">
                      ...
                      <div className="flex-1 h-[100%]">
                        <input
                          ...
                          placeholder="Amount"
                          className="w-[200px] h-[100%] px-[10px] placeholder:text-xs placeholder:font-normal placeholder:text-[#A1AAB4]"
                        />
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="w-auto h-[30px] flex justify-start border border-[#D6DDEB] rounded">
                      ...
                      <div className="flex-1 h-[100%]">
                        <input
                          ...
                          placeholder="From"
                          className="w-[150px] h-[100%] px-[10px] placeholder:text-xs placeholder:font-normal placeholder:text-[#A1AAB4]"
                        />
                      </div>
                    </div>
              ...
              <div className="w-full h-[auto] grid grid-rows-[30px_auto] border border-[#D6DDEB] rounded p-[10px]">
                ...
                <div className="w-[100%] min-h-[300px] flex items-center justify-center">
                  <JobDescriptionEditor
                    ...
                    onChange={(htmlValue) =>
                      setValue("jobDescription", htmlValue, {
                        shouldValidate: true,
                        shouldDirty: true,
                      })
                    }
